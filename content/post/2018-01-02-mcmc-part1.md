...
3. The chain will converge to the target distribution if the transition probability is:
    * *irreducible*: For any state of the chain, there is a non-zero probability of moving to any other point in the space. This means that our transition probability distribution must have non-zero probability across the entire parameter space.
